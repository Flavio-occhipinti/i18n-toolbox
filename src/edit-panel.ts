...
export class LangFormPanel {
    ...
    private constructor(panel: WebviewPanel, extensionPath: string, langsText?: any, keysLangs?: KeysLangs) {
        this._panel = panel;
    ...
    public static createOrShow(extensionPath: string, langsText?: LanguageText[], keysLangs?: KeysLangs) {
        ...
        if (LangFormPanel.currentPanel) {
            ...
            const panel = window.createWebviewPanel('ts', 'i18n Toolbox', ViewColumn.Three, {
                // Enable javascript in the webview
                enableScripts: true,
                localResourceRoots: [Uri.file(path.join(extensionPath, 'ressources'))],
            });
    ...
    private _update(langsText?: LanguageText[], keysLangs?: KeysLangs) {
        if (langsText) {
            this._panel.webview.html = this._getLangsTextView(langsText);
        } else {
            const ressourceSrc = Uri.file(path.join(this._extensionPath, 'ressources')).with({ scheme: 'vscode-resource' }).toString() + '/';
    ...
    private _getEmptyKeysView(keysLangs: KeysLangs = {}, ressourceSrc: string) {
...
<div class="container">
    ...
    ${langs
        .map(
            (lang) =>
                `<div class="lang-wrapper"><div class="lang">${lang}</div><div class="keys">${keysLangs[lang]
                    .map(
                        ({ key, line, jsonPath }) =>
                            `<div class="key">${key} <button onclick="openFile('${lang}','${key}',${line})"><img src="external-link.png" alt="external link" /></button> <button onclick="openEditValue('${jsonPath}')"><img  src="pencil.png" alt="edit" /></button></div>`
                    )
                    .join('')}</div></div>`
        )
        .join('')}
    ...
    private _getLangsTextView(langsText: LanguageText[]) {
